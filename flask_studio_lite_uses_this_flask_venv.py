...
class UniversalFlaskStudio:
    ...
    def create_widgets(self):
        ...
        # Flask App Setting
        ttk.Label(main_frame, text="Flask App:",
        ...
        ttk.Entry(app_frame, textvariable=self.flask_app_var).grid(
            row=0, column=0, sticky="ew", padx=(0, 10))
        ttk.Button(app_frame, text="Auto-Detect",
                   command=self.auto_detect_flask_app).grid(row=0, column=1)

        # Python Path
        ttk.Label(main_frame, text="Python Executable:",
        ...
        ttk.Entry(python_frame, textvariable=self.python_var).grid(
            row=0, column=0, sticky="ew", padx=(0, 10))
        ttk.Button(python_frame, text="Auto-Detect",
                   command=self.detect_python).grid(row=0, column=1)
        ...
        ttk.Label(options_frame, text="Port:",
                  font=("Arial", 10, "bold")).grid(row=0, column=0, sticky="w", padx=(0, 10))

        self.port_var = tk.StringVar(value="5000")
        ttk.Spinbox(options_frame, from_=5000, to=9999,
                    textvariable=self.port_var, width=8).grid(row=0, column=1, padx=(0, 20))

        self.auto_port_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Auto-find available port",
                        variable=self.auto_port_var).grid(row=0, column=2, sticky="w")
        ...
        ttk.Checkbutton(debug_frame, text="Debug mode",
                        variable=self.debug_var).grid(row=0, column=0, sticky="w", padx=(0, 20))

        self.reload_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(debug_frame, text="Auto-reload on changes",
                        variable=self.reload_var).grid(row=0, column=1, sticky="w")

        # Host option
        host_frame = ttk.Frame(main_frame)
        host_frame.grid(row=6, column=0, columnspan=3, sticky="ew", pady=5)

        ttk.Label(host_frame, text="Host:",
                  font=("Arial", 10, "bold")).grid(row=0, column=0, sticky="w", padx=(0, 10))

        self.host_var = tk.StringVar(value="127.0.0.1")
        host_combo = ttk.Combobox(host_frame, textvariable=self.host_var,
                                  values=["127.0.0.1", "localhost", "0.0.0.0"], width=15)
        ...
        self.test_btn = ttk.Button(button_frame, text="🧪 Test Config",
        ...
        self.url_label = tk.Label(status_frame, text="", fg="blue",
        ...
        ttk.Button(console_controls, text="Clear",
    ...
    def auto_detect_flask_app(self):
        ...
        if flask_files:
            ...
            for flask_file in flask_files:
                test_patterns = [f"{flask_file}:app",
                                 f"{flask_file}:application",
                                 f"{flask_file}:create_app"]
    ...
    def detect_python(self):
        """Auto-detect Python executable (prioritize Flask Studio's venv first)"""
        self.log("🐍 Detecting Python executable...")

        # Get Flask Studio's directory (where this script is located)
        flask_studio_dir = os.path.dirname(os.path.abspath(__file__))

        # First, check for Flask Studio's own venv
        flask_studio_venv_patterns = ['.venv', 'venv']

        for pattern in flask_studio_venv_patterns:
            venv_path = os.path.join(flask_studio_dir, pattern)

            # Windows
            python_exe = os.path.join(venv_path, 'Scripts', 'python.exe')
            if os.path.exists(python_exe):
                self.python_var.set(python_exe)
                self.log(f"✅ Using Flask Studio's virtual environment: {pattern}")
                return
        ...
        # Second, check for project's virtual environments (if project path is set)
        if self.project_path:
            ...
            for pattern in venv_patterns:
                ...
                if os.path.exists(python_exe):
                    self.python_var.set(python_exe)
                    self.log(f"✅ Found project's virtual environment: {pattern}")
                    return
